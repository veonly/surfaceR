...
days <- c(1:31) # user choice. delete if runs 365 days 
...
# Set up loops in parallel
foreach (day = c(1:31)) %dopar% { #user choice. use 31 for now, should be 1:365
...
# Set up loops in parallel
foreach(day = days) %dopar% {   # day-specific variance for first stage spatial smooths
...
############################ 4. MODEL OUTPUT #############################
write.csv(valid,"fullValid_london_JAN.csv")
gamPred_LON <- cbind.data.frame(valid$X, valid$Y, valid$dayID, valid$validPred)

save.image("/rds/general/user/ww3414/ephemeral/RWorkspace/SurfaceDev_HPC_London_Result_JAN.RData")
